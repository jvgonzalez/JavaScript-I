...
console.log(`Car number 33 is a ${inventory[34].car_year} ${inventory[34].car_make}`);


// ==== Challenge 2 ====
// The dealer needs the information on the last car in their inventory.  What is the make and model of the last car in the inventory?  Log the make and model into the console.
let lastCar = inventory[inventory.length -2];
...
let carModels = [];
const modelSort = inventory.forEach(function(car){
...
let carYears = [];
const yearsort = inventory.forEach(function(CarYear){
    carYears.push(CarYear.car_year);
})
...
let oldCars = [];
const reallyOldCars = carYears.map(function(CarYear){
    if (CarYear < 2000){
...
let BMWAndAudi = [];
const LuxuryCars = inventory.map(function(lux){
    if ((lux.car_make === "BMW" || lux.car_make === "Audi")) {
        BMWAndAudi.push(`${lux.car_model} ${lux.car_make}`)
    } 
})
